...
const getQuizPath = (activityName: string, study: IQuizStudyRef): string => {
  let typeName = ''
  switch (activityName) {
...
const getRecordPath = (step: string, study: IQuizStudyRef): string => {
  const { studyId, studentHistoryId } = study
  const params = `studentHistoryId=${studentHistoryId}&studyId=${studyId}`
  return `${GET_RECORD_DATA_PATH}/${step}?${params}`
}
export { getQuizPath, getRecordPath, getStudyInfoPath }
